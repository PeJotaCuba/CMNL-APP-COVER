...
const targetFunc = 'export const checkSigningAuthorization = (userId: string) => {';
const startIndex = code.indexOf(targetFunc);

const fullReplacement = `export const checkSigningAuthorization = (userId: string) => {
...
if (startIndex > -1) {
    code = code.substring(0, startIndex) + fullReplacement;
    fs.writeFileSync(path, code);
    console.log("Patched full function");
} else {
    console.log("Could not find checkSigningAuthorization");
}
