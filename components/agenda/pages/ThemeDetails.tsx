...
import { generateThemeIdeas } from '../services/ideasService';
...
const ThemeDetails: React.FC<ThemeDetailsProps> = ({ user, onMenuClick, onBack }) => {
  ...
  const loadIdeas = async () => {
    ...
    setErrorMsg('');
    try {
      const text = await generateThemeIdeas(theme, program || "Programa RCM", instructions || "");
      setIdeasContent(text);
    } catch (error: any) {
      setErrorMsg(error.message || "Error al conectar con el servicio de ideas.");
    } finally {
      setLoading(false);
    }
  };
